...
import { parseOptionalPositiveInt } from "../utils/validation";
...
export const getOverview = async (
  ...
  const identifiantEntreprise =
    user?.identifiantEntreprise ||
    (entrepriseId ? String(entrepriseId) : undefined);

  const annee = parseOptionalPositiveInt(
    req.query["annee"],
  ...
  const totalEmployes = await prisma.user.count({
    where: entrepriseId
      ? { entrepriseId }
      : { role: { in: ["EMPLOYE", "MANAGER", "CONSULTANT"] } },
  });
  ...
  const totalBudgetAnnee = budgetsAnnuels.reduce(
    (sum, b) => sum + b.budget.toNumber(),
    0,
  );
  const totalMontantRestantAnnee = budgetsAnnuels.reduce(
...
export const getGlobalAnalytics = async (
  ...
  const entreprisesActives = await prisma.entreprise.count({
  ...
  const totalBudgetDepartements = budgetsDepartements.reduce(
    (sum, b) => sum + b.montant_alloue.toNumber(),
    0,
  );
  const totalBudgetDepartementsUtilise = budgetsDepartements.reduce(
    (sum, b) => sum + b.montant_utilise.toNumber(),
    0,
  );
  const totalBudgetDepartementsRestant = budgetsDepartements.reduce(
    (sum, b) => sum + b.montant_restant.toNumber(),
    0,
  );

  const totalBudgetPersonnels = budgetsPersonnels.reduce(
    (sum, b) => sum + b.montant_alloue.toNumber(),
    0,
  );
  const totalBudgetPersonnelsUtilise = budgetsPersonnels.reduce(
    (sum, b) => sum + b.montant_utilise.toNumber(),
    0,
  );
  const totalBudgetPersonnelsRestant = budgetsPersonnels.reduce(
    (sum, b) => sum + b.montant_restant.toNumber(),
    0,
  );

  const budgetsDepartementsBloques = budgetsDepartements.filter(
    (b) => b.bloquer,
  ).length;
  const budgetsPersonnelsBloques = budgetsPersonnels.filter(
    (b) => b.bloquer,
  ).length;
